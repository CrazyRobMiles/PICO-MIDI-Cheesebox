...
class Rhythm(Voice):
    ...
    def __init__(self,client,col,chan):
        ...
        self.factor=0
        self.track = (
            Step("s1",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s2",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s3",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s4",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s5",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s6",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s7",Rhythm.rhythm_cols, Rhythm.index,0),
            Step("s8",Rhythm.rhythm_cols, Rhythm.index,0)
            )
        self.settings = (
    ...
    def note_down(self,note):
        if note.num<len(self.track):
            self.track[note.num].step()
            self.client.pixels[note.pixel]=self.track[note.num].get_colour()
    ...
    def last_note_off(self):
        if self.need_redraw and not self.client.settings_active and self.client.target==self:
            pixel = self.client.notes[self.sequence_pos].pixel
            col = self.track[self.sequence_pos].get_colour()            
            self.client.pixels[pixel]=col
        if self.last_note != -1:
    ...
    def update(self):
        ...
        if self.sequencing:
            if self.client.time >= self.next_note:
                ...
                self.factor=0    
                self.last_note_off()
                seq_length = self.settings[Rhythm.set_length].get_value()
...
class MidiBox():
    ...
    def sel_down(self, select):
        ...
        if select.col != self.target.col:
            ...
            self.select_pixel = select.pixel
            self.pixels[select.pixel]=select.col
        else:
            self.target.show_settings()
    ...
    def __init__(self):
        ...
        for select in self.selects:
            self.pixels[select.pixel]=Col.BLACK
        self.settings_active = False
        self.select_voice(self.sounds[0].col)
        self.pixels[self.selects[0].pixel] = self.sounds[0].col
